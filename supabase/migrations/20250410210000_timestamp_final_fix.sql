...
-- Final unified fix for timestamp comparison issues

-- Drop the view if it exists and recreate with proper timestamp handling
DROP VIEW IF EXISTS public.pending_scheduled_files;

-- Create consistent view with explicit casting for timestamp comparisons
...
SELECT 
  ...
  error_message,
  email_id,
  -- Ensure consistent timestamp type for comparison
...
-- Update the functions for consistent timestamp handling
...
BEGIN
  ...
  VALUES (
    ...
    p_storage_path,
    p_recipient_email,
    -- Explicit casting to ensure timestamp with time zone
...
-- Update the update function with proper timestamp handling
...
-- Fix any existing scheduled_date values
UPDATE public.scheduled_files
